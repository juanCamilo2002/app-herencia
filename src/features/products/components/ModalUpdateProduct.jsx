...
const ModalUpdateProduct = ({open, onClose, product}) => {
  const handleClose = () => {
    if (onClose) {
      onClose();
    }
          ...
          <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
            ...
            <div className="mt-2">
              <p className="text-sm dark:text-gray">
                Llena los campos para editar Producto.
              </p>
            </div>
          </div>
        </div>
      </div>
      <FormProduct onClose={handleClose} initialValues={product} />
    </ModalContainer>
  );
};
